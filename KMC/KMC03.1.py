...
Lattice = np.zeros([NL,NL,NL])
energy_term = np.zeros([NL,NL,NL])
        ...
        for k in range (NL):
            if (temp_NA > 0):
                ...
                if (r1 < 0.1):
                    ...
                    Lattice[i][j][k] = 1
            energy_term[i][j][k] = kbT * random.uniform(-1.0,1.0)
...
# x,y,z coordinates
pos_i = np.zeros((3,), dtype=int) #initial position = (1,1,1)
pos_i[0] = 1
pos_i[1] = 1
pos_i[2] = 1
pos_j = np.zeros((3,), dtype=int) #position at any time t
pos_j[0] = NL/2
pos_j[1] = NL/2
pos_j[2] = NL/2
pos_f = np.zeros((3,), dtype=int) #final position
total_x2 = 0
D = 0
t = 0

NSteps = 100
pos_traj = np.zeros([3,NSteps])
pos_traj[0][0] = pos_j[0]
pos_traj[1][0] = pos_j[1]
pos_traj[2][0] = pos_j[2]
energy_direc = np.zeros(6)
rate_direc = np.zeros(6)
...
Lattice[pos_j[0]][pos_j[1]][pos_j[2]] = 2
...
for i in range (1, NSteps):
    ...
    if (Lattice[pos_j[0]-1][pos_j[1]][pos_j[2]] != 1) :
        energy_direc[0] = energy_term[pos_j[0]-1][pos_j[1]][pos_j[2]] - energy_term[pos_j[0]][pos_j[1]][pos_j[2]]
    else :
        energy_direc[0] = math.inf
    if (Lattice[pos_j[0]+1][pos_j[1]][pos_j[2]] != 1) :
        energy_direc[1] = energy_term[pos_j[0]+1][pos_j[1]][pos_j[2]] - energy_term[pos_j[0]][pos_j[1]][pos_j[2]]
    else :
        energy_direc[1] = math.inf
    if (Lattice[pos_j[0]][pos_j[1]-1][pos_j[2]] != 1) :
        energy_direc[2] = energy_term[pos_j[0]][pos_j[1]-1][pos_j[2]] - energy_term[pos_j[0]][pos_j[1]][pos_j[2]]
    else :
        energy_direc[2] = math.inf
    if (Lattice[pos_j[0]][pos_j[1]+1][pos_j[2]] != 1) :
        energy_direc[3] = energy_term[pos_j[0]][pos_j[1]+1][pos_j[2]] - energy_term[pos_j[0]][pos_j[1]][pos_j[2]]
    else :
        energy_direc[3] = math.inf
